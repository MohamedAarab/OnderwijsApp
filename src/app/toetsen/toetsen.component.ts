...
export class ToetsenComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    //       for(let i = 0; i < this.courses[j].toetsen.length; i++){
    //         this.cursusService.getDataByHref(this.courses[j].toetsen[i].href)
    //           .subscribe(toetsen => {
    //
    //               this.courses[j].toetsen[i].data = toetsen;
    //               if(this.courses[j].toetsen[i].data.bloomniveau != null) {
    //                 this.cursusService.getDataByHref(this.courses[j].toetsen[i].data.bloomniveau.href)
    //                   .subscribe(bloomniveau => {
    //                     this.courses[j].toetsen[i].data.bloomniveau.data = bloomniveau;
    //                   });
    //               }
    //             },
